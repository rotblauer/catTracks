...
function gotData(data) {
    ...
    console.log("allteamcount: ", data["all"]["team"]["count"]);
    $("#overview").html("<p>" + parseInt(data["all"]["team"]["count"]).toLocaleString("en-US") + " total points" + "</p>");
    console.log("localerized", parseInt(data["all"]["team"]["count"]).toLocaleString("en-US"));
    ...
    for (i in cats) {
        ...
        for (i in data) {
            ...
            col.attr("id", (cat + i).replace(' ', "").replace("'", ""));
            ...
            makeSpeedometer((cat + i).replace(' ', "").replace("'", ""), Math.floor(tp.speed_stats.max), Math.floor(tp.speed_stats.mean), distance);
